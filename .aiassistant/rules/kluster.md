...
- **CRITICAL**: You have to read tool schema file to understand tool parameters before calling kluster_code_review_auto. To check yourself - "Have I read kluster_code_review_auto tool schema already?"
...
## Planning Mode
- When in planning mode, always include kluster.ai code verification `kluster_code_review_auto` in the final plan
- When user requests to pause or resume kluster, include `kluster_review_pause` tool call in the plan — never plan to delete or modify rules files as a way to pause
